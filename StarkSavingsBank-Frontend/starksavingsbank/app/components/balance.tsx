...
import { ArrowLeft, Coins, Scroll, ArrowDownRight, ArrowUpRight, Shield } from 'lucide-react'
  ...
  const fetchTransactions = async () => {
    if(!authInfo?.loading) {
      if(authInfo?.isAuthenticated) {
        ...
        axios.get(`${AppConstants.ROOT_URL+AppConstants.BALANCE_API_URL}?id=${authInfo.userDetails.id}`, {
          headers: {
          ...
          console.log("An error occured while fetching balance details due to : " + error)
          setLoading(false)
        })
      } else {
        redirect()
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 text-gray-100 p-4 sm:p-8">
      <div className="max-w-6xl mx-auto">
                    ...
                    {loading ? (
                      ...
                      >
                        <td colSpan={4} className="p-3 text-center">
                          <span className="animate-pulse">The ravens are fetching your scrolls...</span>
                        </td>
                      </motion.tr>
                    ) : (
                      transactions.map((transaction, index) => (
                        ...
                        >
                          <td className="p-3">{convertToGotDate(transaction.transactionDt)}</td>
                          <td className="p-3">{transaction.transactionSummary}</td>
                          <td className="p-3 text-right">
                            <span className={`flex items-center justify-end ${transaction.transactionType === 'Withdrawal' || transaction.transactionType === 'Transfer' ? 'text-red-400' : 'text-green-400'}`}>
                          ...
                          <td className="p-3 text-right">{formatCurrency(transaction.closingBalance)}</td>
                        </motion.tr>
                      ))
                    )}
                  </AnimatePresence>
        ...
        >
          <Link href="/dashboard" className="inline-flex items-center px-6 py-3 bg-gray-700 hover:bg-gray-600 text-white rounded-md transition-colors text-lg">
        ...
        >
          <p>Remember, a Lannister always pays their debts, but a wise Stark saves for winter.</p>
        </motion.div>
      </div>
    </div>
  )
